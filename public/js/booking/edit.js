class BookingForm {
    constructor(formConfig) {
        ...
        // Inicjalizacja z danymi początkowymi dla trybu edycji
        if (formConfig.initialData) {
            this.initialData = formConfig.initialData;
        } else {
            console.error('Brak danych początkowych dla trybu edycji!');
        }
    ...
    setupDateNavigation() {
        ...
        if (!document.getElementById('datesContainer')) {
            ...
            // Ustaw datę początkową na datę edytowanej wizyty
            if (this.initialData && this.initialData.bookingDate) {
                this.currentDate = new Date(this.initialData.bookingDate);
            } else {
                this.currentDate = new Date();
            }
            this.selectedDate = null;
            
            // Generuj daty zaczynając od daty wizyty
    ...
    initializeEventListeners() {
        ...
        // Ustaw początkowego pracownika, jeśli dostępny
        if (this.initialData && this.initialData.employeeId) {
            this.employeeSelect.value = this.initialData.employeeId;
        }
    }
    ...
    // Funkcja generująca daty - zaczynając od daty edytowanej wizyty
    generateDates(startDate) {
        if (!this.datesContainer) return;
        
        this.datesContainer.innerHTML = '';
        
        // Używamy przekazanej daty startowej jako bazy dla pierwszego dnia
        const firstDay = new Date(startDate);
        
        // Tylko przy pierwszym ładowaniu ustawiamy datę edytowanej wizyty jako pierwszą
        // Sprawdzamy czy to pierwsze ładowanie (selectedDate jest null)
        if (this.initialData && this.initialData.bookingDate && this.selectedDate === null) {
            const bookingDate = new Date(this.initialData.bookingDate);
            // Data wizyty będzie pierwsza z lewej
            firstDay.setTime(bookingDate.getTime());
            // Aktualizujemy currentDate, aby przyciski nawigacji działały relatywnie do tej daty
            this.currentDate = new Date(bookingDate);
        }
        ...
        for (let i = 0; i < 7; i++) {
            ...
            // Jeśli to data edytowanej wizyty, oznacz ją
            if (this.initialData && this.initialData.bookingDate) {
                const bookingDate = new Date(this.initialData.bookingDate);
        ...
        // Jeśli to pierwsze generowanie dat i mamy datę wizyty, zaznacz ją
        if (this.selectedDate === null && this.initialData && this.initialData.bookingDate) {
            setTimeout(() => {
                const dateCard = this.datesContainer.querySelector(`[data-date="${this.initialData.bookingDate}"]`);
                if (dateCard && !dateCard.classList.contains('disabled')) {
                    document.querySelectorAll('.date-card').forEach(card => {
                        card.classList.remove('active');
                    });
                    dateCard.classList.add('active');
                    this.selectedDate = this.initialData.bookingDate;
                    this.dateInput.value = this.initialData.bookingDate;
                    this.fetchAvailableSlots();
                }
            }, 100);
        }
    ...
    renderTimeSlots(slots) {
            ...
            if (this.selectedDate === this.initialData.bookingDate) {
                ...
                if (!hasInitialSlot) {
                    ...
                    const newSlot = {
                        ...
                        time: initialTime,
                        current: true // Oznacz jako bieżący slot
                    };
                    ...
                    // Oznacz istniejący slot jako bieżący
                    slots.forEach(slot => {
                        const slotDate = new Date(slot.datetime);
                        if (slotDate.getHours() === initialDateTime.getHours() && 
                            slotDate.getMinutes() === initialDateTime.getMinutes()) {
                            slot.current = true;
                        }
                    });
        ...
        slots.forEach(slot => {
            ...
            if (this.initialData && this.initialData.startTime) {
                const initialDateTime = new Date(this.initialData.startTime);
                if (slotTime.getTime() === initialDateTime.getTime() || slot.current) {
                    slotElement.classList.add('selected');
                    slotElement.classList.add('current'); // Dodaj klasę current dla aktualnej godziny
